let CircleController = function () {
    ...
    for (let i = 0; i < this.feelings.length; i++) {
        ...
        let p = { id: this.feelings[i].getAttribute('id'), x: Math.cos(Math.PI * 2 * i / this.feelings.length)  * this.node.offsetWidth / 2, y: -Math.sin(Math.PI * 2 * i / this.feelings.length)  * this.node.offsetWidth / 2 }
...
CircleController.prototype.step = function (time) {
    ...
    let angleInc = Math.PI * 2 / this.feelings.length;
