...
export class PersonaCore {
  ...
  */
  constructor(scene, settings = null) {
    ...
    //this is where we can get params from the URL to make different defaults.  might be best to create some config file and loader.
    //set the default color scheme (this is hsl value for X)
    this._urlColor=30;

    //check the url for color settings
    var urlParams = new URLSearchParams(window.location.search);
    if (urlParams.get('color')){
      this._urlColor = urlParams.get('color');
    }
    ...
    this._data = {
      ...
      hsl: new THREE.Vector3(this._urlColor, 1, 0.6),
    ...
    // starting randon animations
    this._startRandonAnimations();
  }
  ...
  get randonAnimations() {
    return this._randonAnimations;
  }
  set randonAnimations(value) {
    this._randonAnimations = value;
  }
  ...
  _computeColors() {
    ...
    //console.log(hsl);
    //this is what sets the entire color scheme up.  something sorta weird about this.
  ...
  _startRandonAnimations() {
    if (this._randonAnimations) {
      console.log('starting randon animations... ');

      const statesIndex = randomStates.length - 1;
      console.log(`got ${randomStates.length} states`);

      setInterval(() => {
        setTimeout(() => {
          const random = Math.floor(Math.random() * (statesIndex - 0 + 1) + 0);
          const state = randomStates[random];
          console.log(`got ${state} from random function 10 seconds`);
          // @ts-ignore
          this.setState(state);
        }, 10000);
        setTimeout(() => {
          const random = Math.floor(Math.random() * (statesIndex - 0 + 1) + 0);
          const state = randomStates[random];
          console.log(`got ${state} from random function 20 seconds`);
          // @ts-ignore
          this.setState(state);
        }, 20000);
      }, 30000);
