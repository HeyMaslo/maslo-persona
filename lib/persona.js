import { observable, observe, toJS as mobxToJS } from 'mobx';
...
export class MasloPersona {
    ...
    @observable
    _mood = { anger: 0, calm: 0, joy: 0, love: 0, sadness: 0, sleepy: 0, surprise: 0, terror: 0 };
    ...
    */
    constructor(scene, settings = null) {
        ...
        this._disposeUpdateMoodReaction = observe(this._mood, () => {
            // logger.warn('Changed!');
     ...
     * */
    _setStateInternal(state, force = false, stateArgs = null) {
    ...
    beginState(state) {
        ...
        this._setStateInternal(state, false, continualArgs);

        if (!continualArgs.used) {
            logger.error(`Tried tpo begin state "${state}", but its runner doesn't support it.`);
            this.endState();
        }
    }
